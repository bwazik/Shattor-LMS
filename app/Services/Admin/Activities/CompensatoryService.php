...
class CompensatoryService
{
    ...
    public function insertCompensatory(array $request)
    {
        return $this->executeTransaction(function () use ($request) {
            ...
            $originalLesson = Lesson::with('group')->findOrFail($request['original_lesson_id']);
            $makeupLesson = Lesson::with('group')->findOrFail($request['makeup_lesson_id']);

            // Validate teacher ownership
            if (isset($request['teacher_id']) && $request['teacher_id'])
            {
                $isValidTeacher = $student->teachers()->where('teacher_id', $request['teacher_id'])->exists() &&
                    $originalLesson->group->teacher_id == $request['teacher_id'] &&
                    $makeupLesson->group->teacher_id == $request['teacher_id'];
                if (!$isValidTeacher) {
    ...
    public function deleteCompensatory($id)
    {
        return $this->executeTransaction(function () use ($id) {
            Compensatory::findOrFail($id)->delete();

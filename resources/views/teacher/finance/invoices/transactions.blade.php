...
@section('content')
    ...
    <x-datatable datatableTitle="{{ trans('main.datatableTitle', ['item' => trans('admin/transactions.transactions')]) }} - {{ $invoice->student->name ?? 'N/A' }} - {{ $invoice->fee->name ?? 'N/A' }} - {{ $invoice->student->grade->name ?? 'N/A' }}">
        <th></th>
...
@section('page-js')
    <script>
        initializeDataTable('#datatable', "{{ route('teacher.invoices.transactions', $invoice->uuid) }}", [2, 3, 4, 5, 6, 7],
            [

...
@section('content')
    ...
    <x-modal modalType="reset-offline-quiz" modalTitle="{{ trans('admin/quizzes.resetStudentQuiz') }}" id
        action="{{ route('teacher.offline-quizzes.resetStudentOfflineQuiz', ['uuid' => $offlineQuiz->uuid]) }}"
...
@section('page-js')
    <script>
        initializeDataTable('#datatable', "{{ route('teacher.offline-quizzes.scores', $offlineQuiz->uuid) }}", [1, 2, 3,
            4],
            [{
                    data: "",
                    orderable: false,
        ...
        $(document).ready(function() {
            let dataTable = null;
            const url = "{{ route('teacher.offline-quizzes.scores.insert', $offlineQuiz->uuid) }}";

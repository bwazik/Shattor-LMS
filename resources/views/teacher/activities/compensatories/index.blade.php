...
@section('content')
    ...
    @include('teacher.activities.compensatories.modals')
    <!--/ DataTable with Buttons -->
@endsection

@section('page-js')
    <script>
        initializeDataTable('#datatable', "{{ route('teacher.compensatories.index') }}", [2, 3, 4, 5, 6, 7, 8],
            [
        ...
        let fields = ['student_id', 'original_lesson_group_id', 'original_lesson_id', 'makeup_lesson_group_id', 'makeup_lesson_id', 'reason'];
        ...
        fetchMultipleDataByAjax('#add-form #student_id', "{{ route('teacher.fetch.students.groups', '__ID__') }}",
            '#add-form #original_lesson_group_id', 'student_id', 'GET');
        fetchMultipleDataByAjax('#add-form #original_lesson_group_id', "{{ route('teacher.fetch.groups.lessons', '__ID__') }}",
            '#add-form #original_lesson_id', 'original_lesson_group_id', 'GET');
        fetchMultipleDataByAjax('#add-form #student_id', "{{ route('teacher.fetch.students.groups', '__ID__') }}?type=makeup",
            '#add-form #makeup_lesson_group_id', 'student_id', 'GET');
        fetchMultipleDataByAjax('#add-form #makeup_lesson_group_id', "{{ route('teacher.fetch.groups.lessons', '__ID__') }}",

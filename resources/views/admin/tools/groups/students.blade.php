...
@section('content')
    ...
    <x-datatable datatableTitle="{{ trans('main.datatableTitle', ['item' => trans('admin/students.students')]) }} - {{ $group->teacher->name }} - {{ $group->grade->name }} - {!! $group->name !!}"
...
@section('page-js')
    <script>
        initializeDataTable('#datatable', "{{ route('admin.groups.students', $group->id) }}", [2, 3, 4, 5, 6, 7],
            [

<!-- Add Offcanvas -->
<x-modal modalType="add" modalSize="modal-lg" modalTitle="{{ trans('main.addItem', ['item' => trans('admin/offlineQuizzes.offlineQuiz')]) }}" action="{{ route('admin.offline-quizzes.insert') }}">
    <div class="row g-5">
        ...
        <x-basic-input context="modal" type="text" name="conducted_at" classes="flatpickr-date" label="{{ trans('main.conducted_at') }}" placeholder="YYYY-MM-DD" value="{{ now()->format('Y-m-d') }}"/>
    </div>
</x-modal>
<!-- Edit Offcanvas -->
<x-modal modalType="edit" modalSize="modal-lg" modalTitle="{{ trans('main.editItem', ['item' => trans('admin/offlineQuizzes.offlineQuiz')]) }}" action="{{ route('admin.offline-quizzes.update') }}" id>
    <div class="row g-5">
...
<x-modal modalType="delete" modalTitle="{{ trans('main.deleteItem', ['item' => trans('admin/offlineQuizzes.offlineQuiz')]) }}"
    action="{{ route('admin.offline-quizzes.delete') }}" id submitColor="danger" submitButton="{{ trans('main.yes_delete') }}">
...
<x-modal modalType="delete-selected" modalTitle="{{ trans('main.deleteItem', ['item' => trans('admin/offlineQuizzes.selectedOfflineQuizzes')]) }}"
    action="{{ route('admin.offline-quizzes.deleteSelected') }}" ids submitColor="danger" submitButton="{{ trans('main.yes_delete') }}">
    @include('partials.delete-modal-body')
</x-modal>

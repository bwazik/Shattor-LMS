...
@section('content')
    ...
    <x-datatable datatableTitle="{{ trans('main.datatableTitle', ['item' => trans('admin/invoices.invoices')]) }}"
        ...
        hrefButtonRoute="{{ route('admin.invoices.create') }}">
        <th></th>
        <th>#</th>
    ...
    @include('admin.finance.invoices.modals')
    <!--/ DataTable with Buttons -->
@endsection

@section('page-js')
    <script>
        initializeDataTable('#datatable', "{{ route('admin.invoices.index') }}", [2, 3, 4, 5, 6, 7],
            [
        ...
        setupModal({
            ...
            fields: {
                id: button => button.data('id'),
                itemToArchive: button => `${button.data('fee')} - ${button.data('student')}`
            }
        });
